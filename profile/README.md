...
WebCross is a desktop application, not a hosted cloud browser. The page engine, the browser instances and your logged-in sessions all live **on your own device** — where cloud browser infrastructure would keep those sessions on someone else's machine and meter them through its gateway.

- **Credentials and sessions stay local** — account credentials and cookie sessions are stored and isolated per task instance on your machine
- **Instances are physically separated** — session state and local storage are isolated, so accounts never bleed into one another
- **Run records are yours** — execution history is retained locally for audit and replay
- **Results land in your filesystem** — the agent organizes what it collects and exports it straight to local files
- **Sensitive steps wait for you** — captcha, 2FA and payment confirmation pause for a human instead of being waved through automatically

That is what we mean by **critical tasks, under your control**.
...
WebCross 是一款桌面端应用，而不是托管在云上的浏览器服务。页面引擎、浏览器实例与你的登录态都运行在**你自己的设备上**——换作云端浏览器基础设施，这些会话会被托管在别人的机器上，并按其网关计费。

- **凭证与会话留在本地** —— 账号凭证与 Cookie 会话按任务实例存储并隔离在你的机器上
- **实例之间物理隔离** —— 会话状态与本地存储彻底解耦，多账号并行不会串号
- **执行记录归你所有** —— 运行历史本地留存，审计与回溯都不必把数据交出去
- **结果直接落到本地** —— 智能体整理完采集内容后，直接导出到你的文件系统
- **敏感环节等你确认** —— 验证码、2FA 与支付确认交由人工把关，而不是被自动放行
...
**Aitheris** · Make the web actually navigable for agents · 让智能体真正跑得动网页
